...
export class Aria2Client {
    ...
    constructor(config: Aria2ClientConfig = {}) {
        ...
        this.http = axios.create({
            timeout: this.timeoutMs > 0 ? this.timeoutMs : 0,
            headers: {
    ...
    private async post<TResponse>(body: unknown): Promise<TResponse> {
        try {
            const response = await this.http.post<TResponse>(this.url, body, {
                validateStatus: () => true,
            });
    ...
    private toHttpError(error: AxiosError): Aria2HttpError {
